...
```{r}
shape <- c(4, 3, 2)
array_index <- rep(1, length(shape))
len_data <- 24
...
- `empty()`: creates an array with NA
- `zeros()`: creates an array with 0
- `ones()`: creates an array with 1
- `full()`: creates an array with value(s)
...
- `delete()`: deletes an axis of an array
